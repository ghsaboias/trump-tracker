...
from summarizer import (
    summarize_text,
    format_summary,
)  # import our summary helper and format_summary
...
def index():
    ...
    if os.path.exists(EXECUTIVE_ORDERS_DIR):
        for filename in os.listdir(EXECUTIVE_ORDERS_DIR):
            if filename.endswith(".json"):
                ...
                try:
                    ...
                    doc_number = order_data.get("data", {}).get(
                        "document_number", "Unknown"
                    )
                    title = order_data.get("data", {}).get("title", "No Title")
                    publication_date = order_data.get("data", {}).get(
                        "publication_date", "N/A"
                    )
                    signing_date = order_data.get("content", {}).get(
                        "signing_date", "N/A"
                    )
                    orders.append(
                        {
        ...
        logging.warning(
            "Executive orders directory does not exist: %s", EXECUTIVE_ORDERS_DIR
        )
    ...
    if sort_by in ("signing_date", "publication_date"):
        orders = sorted(
            orders,
            key=lambda order: order.get(sort_by, ""),
            reverse=(sort_order == "desc"),
        )

    logging.info("Loaded %s executive orders from cache.", len(orders))
    return render_template(
        "index.html", orders=orders, sort_by=sort_by, sort_order=sort_order
    )
...
def order_detail(document_number):
    ...
    if not os.path.exists(filepath):
        logging.error(
            "Requested executive order %s not found in cache.", document_number
        )
        abort(404)
    try:
        with open(filepath, "r") as f:
    ...
    return render_template(
        "order_detail.html", order=order_data, summary="Loading summary..."
    )
...
def summarize_order(document_number):
    ...
    if not os.path.exists(filepath):
        logging.error(
            "Requested executive order %s not found for summarization.", document_number
        )
        abort(404)
    try:
        with open(filepath, "r") as f:
